@if ($errors->any())
    <div class="alert alert-danger d-flex align-items-start gap-2 mb-4" role="alert">
        ...
        <div>
            <div class="fw-semibold">Please review the maintenance record information.</div>
...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select class="form-select" id="maintenance_type" name="maintenance_type" required>
            ...
            <option value="routine"
                {{ old('maintenance_type', $maintenance->maintenance_type ?? '') == 'routine' ? 'selected' : '' }}>
                Routine</option>
            <option value="breakdown"
                {{ old('maintenance_type', $maintenance->maintenance_type ?? '') == 'breakdown' ? 'selected' : '' }}>
                Breakdown</option>
            <option value="accident"
                {{ old('maintenance_type', $maintenance->maintenance_type ?? '') == 'accident' ? 'selected' : '' }}>
                Accident</option>
            <option value="inspection"
                {{ old('maintenance_type', $maintenance->maintenance_type ?? '') == 'inspection' ? 'selected' : '' }}>
                Inspection</option>
        </select>
    </div>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select class="form-select" id="status" name="status" required>
            ...
            <option value="in_progress"
                {{ old('status', $maintenance->status ?? 'in_progress') == 'in_progress' ? 'selected' : '' }}>In
                Progress</option>
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="datetime-local" class="form-control" id="start_datetime" name="start_datetime"
            value="{{ old('start_datetime', $maintenance && $maintenance->start_datetime ? $maintenance->start_datetime->format('Y-m-d\TH:i') : '') }}"
            required>
    </div>
    ...
    <div class="col-md-12 mb-3">
        <label class="form-label" for="service_description">Service Description <span
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-2">
                            <label class="form-label d-flex justify-content-between align-items-center w-100">
                                <span>Part <span class="text-danger">*</span></span>
                                <a href="{{ route('maintenance.parts.create') }}" target="_blank"
                                    class="text-success fs-xs fw-semibold" style="text-decoration: none;">
                                    <i class="fa fa-plus-circle me-1"></i>New Part
                                </a>
                            </label>
                            <select class="form-select js-select2" name="parts[{{ $index }}][vehicle_part_id]"
                                required style="width: 100%;">
                                <option value="">Select Part</option>
                                @foreach ($parts as $part)
                                    <option value="{{ $part->id }}"
                                        {{ $existingPart->vehicle_part_id == $part->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-2 mb-2">
                            ...
                            <select class="form-select" name="parts[{{ $index }}][action_type]">
                                <option value="replace_brand_new"
                                    {{ in_array($existingPart->action_type, ['replace', 'replace_brand_new']) ? 'selected' : '' }}>Replace: Brand New</option>
                                <option value="replace_recondition"
                                    {{ $existingPart->action_type == 'replace_recondition' ? 'selected' : '' }}>Replace: Recondition</option>
                                <option value="repair" {{ $existingPart->action_type == 'repair' ? 'selected' : '' }}>
                                    Repair</option>
                                <option value="service"
                                    {{ $existingPart->action_type == 'service' ? 'selected' : '' }}>Service</option>
                            </select>
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Quantity</label>
                            <input type="number" class="form-control" name="parts[{{ $index }}][quantity]"
                                value="{{ $existingPart->quantity }}" min="1">
                        </div>
                        <div class="col-md-3 mb-2">
                            <label class="form-label">Cost (৳)</label>
                            <input type="number" step="0.01" class="form-control part-cost"
                                name="parts[{{ $index }}][part_cost]" value="{{ $existingPart->part_cost }}">
                        </div>
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            <input type="number" class="form-control"
                                name="parts[{{ $index }}][warranty_period_months]"
                                value="{{ $existingPart->warranty_period_months }}">
                        </div>
                        <div class="col-md-4 mb-2">
                            <label class="form-label">Next Replacement Date</label>
                            <input type="date" class="form-control"
                                name="parts[{{ $index }}][next_replacement_due_date]"
                                value="{{ $existingPart->next_replacement_due_date }}">
                        </div>
                        <div class="col-md-4 mb-2">
                            <label class="form-label">Next Replacement KM</label>
                            <input type="number" class="form-control"
                                name="parts[{{ $index }}][next_replacement_due_km]"
                                value="{{ $existingPart->next_replacement_due_km }}">
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    <div class="part-row border rounded p-3 mb-3">
        <div class="row">
            ...
            <div class="col-md-2 mb-2">
                <label class="form-label">Action</label>
                <select class="form-select" name="parts[INDEX][action_type]">
                    <option value="replace_brand_new">Replace: Brand New</option>
                    <option value="replace_recondition">Replace: Recondition</option>
            ...
            <div class="col-md-2 mb-2">
                <label class="form-label">Quantity</label>
                <input type="number" class="form-control" name="parts[INDEX][quantity]" value="1"
                    min="1">
            </div>
            <div class="col-md-3 mb-2">
                <label class="form-label">Cost (৳)</label>
                <input type="number" step="0.01" class="form-control part-cost" name="parts[INDEX][part_cost]"
                    value="0">
            </div>
            <div class="col-md-1 mb-2 d-flex align-items-end">
            ...
            <div class="col-md-4 mb-2">
                <label class="form-label">Warranty (Months)</label>
                <input type="number" class="form-control" name="parts[INDEX][warranty_period_months]"
                    value="0">
            </div>
            <div class="col-md-4 mb-2">
                <label class="form-label">Next Replacement Date</label>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(".js-select2").select2();
            let partIndex =
                {{ isset($maintenance) && $maintenance->maintenanceParts->count() > 0 ? $maintenance->maintenanceParts->count() : 0 }};
            ...
            $('#addPartBtn').click(function() {
                ...
                // Initialize select2 on the newly added select element only
