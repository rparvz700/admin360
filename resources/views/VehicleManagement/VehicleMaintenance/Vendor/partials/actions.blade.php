<div class="dropdown d-inline-block">
    ...
    <div class="dropdown-menu dropdown-menu-end fs-sm py-1" aria-labelledby="vendorActions{{ $vendor->id }}">
        <!-- View Details -->
        <a class="dropdown-item py-1" href="{{ route('maintenance.vendors.show', $vendor->id) }}">
            <i class="fa fa-eye text-info me-2"></i> View Details
        </a>

        <!-- Edit Vendor -->
        @can('edit-vendor')
            <a class="dropdown-item py-1" href="{{ route('maintenance.vendors.edit', $vendor->id) }}">
        ...
        @can('delete-vendor')
            ...
            <form action="{{ route('maintenance.vendors.destroy', $vendor->id) }}" method="POST" id="vendorDeleteForm{{ $vendor->id }}">
                @csrf
                @method('DELETE')
