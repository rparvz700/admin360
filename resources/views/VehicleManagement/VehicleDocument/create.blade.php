    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">Add Vehicle Document</h3>
                <a href="{{ route('vehicle-documents.index') }}" class="btn btn-sm btn-secondary">Back to List</a>
            </div>
            <div class="block-content">
                <form action="{{ route('vehicle-documents.store') }}" method="POST" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-md-8">
                            @include('VehicleManagement.VehicleDocument.form', ['doc' => null, 'vehicles' => $vehicles, 'categories' => $categories])
                        </div>
                    </div>
