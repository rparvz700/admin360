            ...
            <form action="{{ route('electricity.meters.store') }}" method="POST" autocomplete="off">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="provider_name" name="provider_name" value="{{ old('provider_name') }}" placeholder="e.g. DESCO, DPDC, NESCO, BREB, House Owner">
                    </div>
                        ...
                        <select class="form-select select2" id="building_id" name="building_id" required style="width: 100%;">
                            ...
                            @foreach($buildings as $building)
                                <option value="{{ $building->id }}" {{ old('building_id') == $building->id ? 'selected' : '' }}>
                                    {{ $building->site_name }} {{ ($building->code ?? $building->site_code) ? "(" . ($building->code ?? $building->site_code) . ")" : '' }} {{ $building->rio ? " - RIO: {$building->rio->name}" : '' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
