                ...
                <form action="{{ route('electricity.bills.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select bg-light" id="bill_type" style="pointer-events: none; background-color: #e9ecef;" tabindex="-1" disabled>
                                <option value="postpaid">Postpaid</option>
                                <option value="prepaid">Prepaid</option>
                            </select>
                            <input type="hidden" name="bill_type" id="bill_type_hidden" value="{{ old('bill_type', 'postpaid') }}">
                        </div>
                            ...
                            <select class="form-select select2" id="project_name" name="project_name" required
                                ...
                                @foreach ($projects as $project)
                                    <option value="{{ $project->name }}"
                                        {{ old('project_name', 'BR Project') == $project->name ? 'selected' : '' }}>
                                        {{ $project->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div id="postpaid-section" class="border rounded p-3 mb-4 bg-body-extra-light">
                        <h4 class="fw-light mb-3 text-primary"><i class="fa fa-tachometer-alt me-1"></i> Postpaid Meter
                            Reading & Calculation</h4>
                        <div class="row">
                            <div class="col-md-3 mb-3">
                            ...
                            <div class="col-md-3 mb-3">
                                <label class="form-label" for="current_reading">Current Reading (kWh) <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control calc-input" id="rate_per_unit"
                                    name="rate_per_unit" value="0.00" placeholder="Optional unit rate">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-5 mb-3">
                            <label class="form-label" for="cheque_name">Cheque / Favour Name</label>
                            <input type="text" class="form-control" id="cheque_name" name="cheque_name"
                                value="{{ old('cheque_name') }}" placeholder="e.g. Govt. Revenue Collection">
                        </div>
                    ...
                    <div class="mb-4">
                        <button type="submit" class="btn btn-primary"><i class="fa fa-save me-1"></i> Save Electricity Bill</button>
        ...
        $(document).ready(function() {
            ...
            $('#meter_id').on('change select2:select', function() {
                ...
                if (derivedType === 'prepaid') {
                    ...
                    calculateCalculations('meter_id');
                } else {
            ...
            function calculateCalculations(sourceField) {
                ...
                if (billType === 'postpaid') {
                    ...
                    if (sourceField === 'current_reading' || sourceField === 'rate_per_unit' || sourceField === 'previous_reading') {
                        if (rate > 0 && units >= 0) {
                            net = units * rate;
                            $('#net_amount').val(net.toFixed(2));
                        } else if (net > 0 && units > 0 && rate === 0) {
                            rate = net / units;
                            $('#rate_per_unit').val(rate.toFixed(2));
                        }
                    } else if (sourceField === 'net_amount') {
                        if (units > 0 && net > 0) {
                            rate = net / units;
                            $('#rate_per_unit').val(rate.toFixed(2));
                        }
            ...
            $('#rate_per_unit').on('input change keyup', function() {
                calculateCalculations('rate_per_unit');
            });
