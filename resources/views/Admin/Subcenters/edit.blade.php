    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">Edit {{ $subcenter->name }} Subcenter</h3>
            </div>
            <div class="block-content fs-sm data-content">
                ...
                {{-- Form --}}
                <form class="mb-4" action="{{ route('subcenters.update', $subcenter->id) }}" method="POST"
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-12 mb-4">
                            ...
                            <input type="text" class="form-control" id="subcenter-name" name="name"
                                value="{{ $subcenter->name }}" placeholder="Subcenter Name">
                            @if ($errors->has('name'))
                            ...
                            <select class="js-select2 form-select" id="region" name="region" style="width: 100%;"
                                ...
                                @foreach ($departments as $department)
                                    <option value="{{ $department->id }}"
                                        {{ $department->id == $subcenter->department_id ? 'selected' : '' }}>
                            ...
                            <select class="js-select2 form-select" id="example-select2" name="hod" style="width: 100%;"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ $user->id == $subcenter->head_id ? 'selected' : '' }}>{{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 text-end"> <!-- Use the text-end class to move the button to the right -->
                            <button type="submit" class="btn btn-sm btn-success">Update Subcenter</button>
                        </div>
                    </div>
                </form>
                {{-- End Form --}}
