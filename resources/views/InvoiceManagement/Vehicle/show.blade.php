...
@section('title') Vehicle Invoice Details - {{ $invoice->invoice_number }} @endsection
...
<div class="content">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center py-2 text-center text-md-start mb-3">
        <div class="flex-grow-1 mb-2 mb-md-0">
            <h1 class="h3 fw-bold mb-1">
                <i class="fa fa-car text-primary me-2"></i> Vehicle Invoice: {{ $invoice->invoice_number }}
            </h1>
            <h2 class="h6 fw-medium text-muted mb-0">
                Vehicle maintenance service breakdown and payment history.
            </h2>
        </div>
        <div>
            <a href="{{ route('invoices.vehicle.index') }}" class="btn btn-sm btn-secondary me-1">
                <i class="fa fa-arrow-left me-1"></i> Back to Vehicle Invoices
            </a>
            <a href="{{ route('invoices.vehicle.print', $invoice->id) }}" target="_blank" class="btn btn-sm btn-alt-secondary me-1">
    ...
    <div class="row">
        <!-- Invoice & Vehicle Summary Card -->
        <div class="col-md-8 mb-4">
            <div class="block block-rounded h-100">
                <div class="block-header block-header-default">
                    <h3 class="block-title"><i class="fa fa-tools me-2 text-primary"></i> Vehicle Maintenance Details</h3>
                ...
                <div class="block-content fs-sm">
                    <table class="table table-striped table-borderless">
                        <tbody>
                            ...
                            <tr>
                                <th>Invoice Type</th>
                                <td><span class="badge bg-primary fw-semibold">Vehicle Maintenance</span></td>
                            </tr>
                            <tr>
                                <th>Garage / Vendor</th>
                    ...
                    @if($invoice->maintenances && $invoice->maintenances->count() > 0)
                        <h5 class="mt-4 mb-2"><i class="fa fa-list me-1 text-primary"></i> Linked Maintenance Records</h5>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                ...
                                <tbody>
                                    @foreach($invoice->maintenances as $maint)
                                    <tr>
                                        <td class="fw-bold">{{ $maint->vehicle->registration_number ?? 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-secondary">{{ ucfirst($maint->maintenance_type ?? 'Service') }}</span>
                                        </td>
                                        <td>{{ $maint->service_description ?? 'N/A' }}</td>
                                        <td class="text-end fw-bold">৳ {{ number_format($maint->cost ?? 0, 2) }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
            ...
            <div class="block block-rounded h-100 border-start border-4 border-primary">
                ...
                <div class="block-content fs-sm">
                    <div class="p-3 bg-body-light border rounded mb-3">
                        <div class="fs-xs fw-bold text-uppercase text-muted">Total Maintenance Billed</div>
...
<div class="modal fade" id="modal-record-payment" tabindex="-1" aria-labelledby="modalPaymentLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('invoices.vehicle.pay', $invoice->id) }}" method="POST">
                @csrf
