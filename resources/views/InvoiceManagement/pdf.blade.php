...
<body>
    ...
    <div class="no-print-bar">
        <div>
            ...
            <span style="opacity: 0.7; margin-left: 10px; font-size: 12px;">Official ERP Document</span>
        </div>
        <div>
            <a href="{{ route('invoices.show', $invoice->id) }}" class="btn-back">← Back to Invoice</a>
    ...
    <div class="page-container">
        ...
        <div class="info-grid">
            <!-- Vendor Info -->
            <div class="info-box">
                <div class="info-box-title">Vendor / Billed To</div>
            ...
            <div class="info-box">
                ...
                <div class="info-row">
                    ...
                    <span
                        class="info-value">{{ $invoice->invoice_date ? $invoice->invoice_date->format('d M Y') : 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Payment Due Date:</span>
                    <span
                        class="info-value">{{ $invoice->due_date ? $invoice->due_date->format('d M Y') : 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Payment Status:</span>
                    <span class="info-value">
                        <span class="badge-status badge-{{ $invoice->payment_status }}">
                ...
                @if ($invoice->payment_date)
                    <div class="info-row">
                        ...
                        <span class="info-value">{{ $invoice->payment_date->format('d M Y') }}</span>
                    </div>
                @endif
            </div>
        </div>
        ...
        <table class="table-custom">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <strong style="color: #0f172a;">
                            @if ($invoice->rentBases && $invoice->rentBases->count() > 0)
                                Rent Requisition for Agreement:
                                {{ $invoice->rentBases->first()->agreement->agreement_ref_no ?? 'N/A' }}
                            @elseif($invoice->maintenances && $invoice->maintenances->count() > 0)
                                Vehicle Maintenance Requisition:
                                {{ $invoice->maintenances->first()->vehicle->registration_number ?? 'N/A' }}
                            @else
                                General Vendor Service Invoice
                            @endif
                        </strong>
                        @if ($invoice->remarks)
                            <div style="font-size: 10.5px; color: #64748b; margin-top: 2px;">{{ $invoice->remarks }}
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="text-right fw-bold" style="color: #0284c7;">৳
                        {{ number_format($invoice->total_amount, 2) }}</td>
                </tr>
            </tbody>
        </table>

        <!-- Detailed Rent Breakdown (If linked to Rent record) -->
        @if ($invoice->rentBases && $invoice->rentBases->count() > 0)
                    ...
                    <tbody>
                        @if (isset($rent->components) && $rent->components->count() > 0)
                            @foreach ($rent->components as $comp)
                                <tr>
                                    <td>{{ \App\Services\RentComponentCalculator::COMPONENTS[$comp->component_type]['label'] ?? ucfirst(str_replace('_', ' ', $comp->component_type)) }}
                                    </td>
                                    ...
                                    <td class="text-right fw-bold">৳ {{ number_format($comp->total_amount ?? 0, 2) }}
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                                ...
                                <td class="text-right fw-bold">৳
                                    {{ number_format(($rent->base_rent ?? 0) + ($rent->vat ?? 0) + ($rent->tax ?? 0), 2) }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                        ...
                        @forelse ($utilities as $util)
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ $util->disburse_with_rent ? 'Combined with Rent' : 'Separate Bill' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="text-center" style="color: #64748b;">No utilities
                                    configured.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            @foreach ($rent->increments as $inc)
                                <tr>
                                    <td>{{ $inc->increment_start_date ? \Carbon\Carbon::parse($inc->increment_start_date)->format('d M Y') : 'N/A' }}
                                    </td>
                                    <td>{{ $inc->increment_end_date ? \Carbon\Carbon::parse($inc->increment_end_date)->format('d M Y') : 'N/A' }}
                                    </td>
                                    <td class="text-right">{{ number_format($inc->increment_percentage ?? 0, 2) }}%
                                    </td>
                                    <td class="text-right">৳ {{ number_format($inc->increment_amount ?? 0, 2) }}</td>
                                    <td class="text-right fw-bold">৳
                                        {{ number_format($inc->incremented_amount ?? 0, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            @foreach ($rent->securityDeposits as $dep)
                                <tr>
                                    <td class="text-right fw-bold">৳
                                        {{ number_format($dep->security_deposit_total ?? 0, 2) }}</td>
                                    <td class="text-right">৳
                                        {{ number_format($dep->security_deposit_absorbable ?? 0, 2) }}</td>
                                    <td class="text-right">৳
                                        {{ number_format($dep->security_deposit_non_absorbable ?? 0, 2) }}</td>
                                    <td class="text-center">
                                        {{ $dep->absorb_start_date ? \Carbon\Carbon::parse($dep->absorb_start_date)->format('d M Y') : 'N/A' }}
                                    </td>
                                    <td class="text-center">
                                        {{ $dep->absorb_end_date ? \Carbon\Carbon::parse($dep->absorb_end_date)->format('d M Y') : 'N/A' }}
                                    </td>
        ...
        <div class="financial-summary">
            <div class="words-box">
                <div
                    style="font-weight: 700; font-size: 10.5px; text-transform: uppercase; color: #64748b; margin-bottom: 4px;">
                    Amount in Words
                </div>
            ...
            <div class="summary-box">
                ...
                <div class="summary-row">
                    ...
                    <span style="color: #15803d; font-weight: 600;">৳
                        {{ number_format($invoice->paid_amount, 2) }}</span>
                </div>
                <div class="summary-row">
                    <span>Balance Due:</span>
                    <span style="color: #b91c1c; font-weight: 700;">৳
                        {{ number_format($invoice->getOutstandingAmount(), 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="invoice-footer">
            Computer generated document via Admin360 ERP — Summit Communications Limited. No physical signature
            required.
        </div>
    </div>
</body>
