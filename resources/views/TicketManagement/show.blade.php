                    ...
                    @if(!in_array($ticket->status, ['completed', 'cancelled']))
                        <form action="{{ route('tickets.addUpdate', $ticket) }}" method="POST" class="mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="update_message" rows="3" class="form-control" required placeholder="Write your comment here..."></textarea>
                            </div>
                            ...
                            ">
                                ...
                                @if($update->old_status && $update->new_status)
                                    <small class="text-muted">
                                        Status changed: <strong>{{ ucfirst($update->old_status) }}</strong> → <strong>{{ ucfirst($update->new_status) }}</strong>
                                    </small>
                                @endif
                            </div>
                        @empty
        ...
        <div class="col-lg-4">
            ...
            @if(!in_array($ticket->status, ['completed', 'cancelled']))
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('tickets.cancel', $ticket) }}" method="POST" onsubmit="return confirm('Are you sure you want to cancel this ticket?');">
                            @csrf
